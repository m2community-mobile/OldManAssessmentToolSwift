//
//  EnFindPasswordViewController.swift
//  OldManAssessmentToolSwift
//
//  Created by m2comm on 2024/03/08.
...
class EnFindPasswordViewController: BaseViewController {
...
//        man.titleLabel?.font = UIFont.systemFont(ofSize: 11)
        ...
        man.setImage(UIImage(systemName: "checkmark.circle.fill"), for: .normal)
        man.tag = 1
        ...
        noticeLabel.font = UIFont(name: NanumSquareB, size: 17)
        noticeLabel.textColor = UIColor.white
        noticeLabel.numberOfLines = 0
        noticeLabel.text = "To protect your account, we will verify your identity.\nPlease enter the email address you used to sign up."
        noticeLabel.numberOfLines = 0
        self.view.addSubview(noticeLabel)
        
        //휴대폰번호 입력
        phoneNumberInputView = FPComponentView(placeHoder: "Email", buttonTitle: "Send")
        ...
        noticeLabel2 = UILabel(frame: CGRect(x: 0, y: phoneNumberInputView.frame.maxY + 10, width: SCREEN.WIDTH, height: 80))
        noticeLabel2.textAlignment = .center
        noticeLabel2.font = UIFont(name: NanumSquareB, size: 17)
        noticeLabel2.textColor = UIColor.white
        noticeLabel2.numberOfLines = 0
        noticeLabel2.text = "Check your email for the authentication number."
        noticeLabel2.numberOfLines = 0
       self.view.addSubview(noticeLabel2)
        
        
        //인증번호 입력
        smsInputView = FPComponentView(placeHoder: "Authentication number", buttonTitle: "Confirm")
        smsInputView.frame.origin.y = noticeLabel2.frame.maxY + 10
        ...
        passwordView.setPlaseHolder(plaseHolder: "New Password")
        ...
        passwordView2.setPlaseHolder(plaseHolder: "Password Confirm")
...
//    }
    @objc func requestSMS() {
        ...
        guard let phoneNumber = phoneNumberInputView.textFieldView.textField.text,
            ...
            return appDel.showAlert(title: "Notice", message: "Check email")
...
//        self.timerStart() //test
    ...
    @objc func smsConfirmButtonPressed(){
        ...
        if self.availableTime <= 0 {
            appDel.showAlert(title: "Notice", message: "Get the authentication number first")
            return
        ...
        if self.certificationNumber != "", self.certificationNumber == self.smsInputView.textFieldView.textField.text {
            appDel.showAlert(title: "Notice", message: "Authentication has been completed.")
    ...
    func timeStampUpdate(time : Int){
        if time == 0 {
            ...
            timeStampLabel.text = "Valid time \(minute):\(second)"
        }
    }
        ...
        Server.postData(urlString: urlString, otherInfo: para) { (kData : Data?) in
            if let data = kData {
                if let dataString = data.toString() {
                    ...
                    appDel.showAlert(title: "Notice", message: "Password has been changed.", actions: [UIAlertAction(title: "OK", style: .default, handler: { (action : UIAlertAction) in
                        self.navigationController?.popViewController(animated: true)
                    })], complete: {
...
class FPComponentViewTextFieldView2: TextFieldView {
